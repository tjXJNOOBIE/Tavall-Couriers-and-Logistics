...
public class RoutePlannerService {
    ...
    private final Gemini3TextClient intakeClient;
    private final boolean geminiEnabled;
    private final boolean geminiFlashEnabled;
    private final boolean intakeGeminiEnabled;
    ...
    public RoutePlannerService(ScanErrorCacheService scanErrorCache) {
        ...
        this.intakeClient = buildIntakeClient();
        this.geminiEnabled = this.client != null;
        this.geminiFlashEnabled = this.flashClient != null;
        this.intakeGeminiEnabled = this.intakeClient != null;
    ...
    public IntakeRoutePlan planIntakeAssignment(ShippingLabelMetaDataEntity newLabel,
                                                Map<String, List<ShippingLabelMetaDataEntity>> routes,
                                                double radiusMiles,
                                                int maxStops) {
        if (newLabel == null || isBlank(newLabel.getUuid())) {
            return null;
        }
        if (routes == null || routes.isEmpty()) {
            return new IntakeRoutePlan(null, List.of(newLabel.getUuid()), true, "No existing routes available.");
        }

        if (intakeGeminiEnabled) {
            try {
                IntakeRoutePlan plan = requestIntakeAssignment(newLabel, routes, radiusMiles, maxStops);
                if (plan != null && plan.orderedUuids() != null && !plan.orderedUuids().isEmpty()) {
                    return plan;
                }
            } catch (Exception ex) {
                Log.warn("Intake route planner Gemini failed, using fallback.");
                Log.exception(ex);
            }
        }

        return fallbackIntakeAssignment(newLabel, routes, maxStops);
    }
    ...
    private Gemini3TextClient buildIntakeClient() {
        String apiKey = System.getenv("GEMINI_API_KEY");
        if (apiKey == null || apiKey.isBlank()) {
            Log.warn("Intake route planner disabled: GEMINI_API_KEY missing.");
            return null;
        }
        return new Gemini3TextClient(buildIntakeAssignmentSchema());
    }
    ...
    private IntakeRoutePlan requestIntakeAssignment(ShippingLabelMetaDataEntity newLabel,
                                                    Map<String, List<ShippingLabelMetaDataEntity>> routes,
                                                    double radiusMiles,
                                                    int maxStops) throws Exception {
        String prompt = buildIntakeAssignmentPrompt(newLabel, routes, radiusMiles, maxStops);
        Log.info("Intake route planner Gemini 3 Pro request (radius=" + radiusMiles + ", maxStops=" + maxStops + ")");
        Content content = Content.fromParts(Part.fromText(prompt));
        long startNanos = System.nanoTime();
        GenerateContentResponse response;
        try {
            response = intakeClient.getClient().models.generateContent(
                    String.valueOf(GeminiModel.GEMINI_3_PRO), content, intakeClient.getGenerationConfig());
        } catch (Exception ex) {
            ClientResponseMetadata metadata = ClientResponseVisualizer.buildMetadata(
                    null,
                    GeminiModel.GEMINI_3_PRO,
            ...
            Log.info("[GeminiUsage] IntakeRoutePlan " + ClientResponseVisualizer.format(metadata));
            throw ex;
        }
        ClientResponseMetadata metadata = ClientResponseVisualizer.buildMetadata(
                response,
        ...
        JsonNode root = objectMapper.readTree(jsonText);
        String routeId = root.has("routeId") && root.get("routeId").isTextual()
                ? root.get("routeId").asText()
                : null;
        boolean createRoute = root.has("createNewRoute") && root.get("createNewRoute").asBoolean(false);
        List<String> ordered = new ArrayList<>();
        JsonNode orderNode = root.get("orderedUuids");
        if (orderNode != null && orderNode.isArray()) {
            for (JsonNode node : orderNode) {
        ...
        return new IntakeRoutePlan(routeId, ordered, createRoute, notes);
    }
    ...
    private IntakeRoutePlan fallbackIntakeAssignment(ShippingLabelMetaDataEntity newLabel,
                                                     Map<String, List<ShippingLabelMetaDataEntity>> routes,
                                                     int maxStops) {
        String selectedRoute = null;
        int bestCount = Integer.MAX_VALUE;
        for (Map.Entry<String, List<ShippingLabelMetaDataEntity>> entry : routes.entrySet()) {
            if (entry == null || entry.getKey() == null || entry.getKey().isBlank()) {
                continue;
            }
            List<ShippingLabelMetaDataEntity> stops = entry.getValue();
        ...
        if (selectedRoute == null) {
            return new IntakeRoutePlan(null, List.of(newLabel.getUuid()), true, "Fallback: new route.");
        }
        List<String> ordered = new ArrayList<>();
        List<ShippingLabelMetaDataEntity> stops = routes.get(selectedRoute);
        if (stops != null) {
            for (ShippingLabelMetaDataEntity stop : stops) {
                if (stop != null && !isBlank(stop.getUuid())) {
                    ordered.add(stop.getUuid());
                }
            }
        }
        ordered.add(newLabel.getUuid());
        return new IntakeRoutePlan(selectedRoute, ordered, false, "Fallback: appended intake to route.");
    }
    ...
    private String buildIntakeAssignmentPrompt(ShippingLabelMetaDataEntity newLabel,
                                               Map<String, List<ShippingLabelMetaDataEntity>> routes,
                                               double radiusMiles,
                                               int maxStops) {
        StringBuilder builder = new StringBuilder();
        builder.append("SYSTEM: You are a route optimization agent with access to the Google Maps tool.\n");
        builder.append("TASK: For a newly intaked package, decide whether to add it to an existing route or ");
        builder.append("create a new single-stop route if no existing route fits within a ")
                .append(radiusMiles).append(" mile radius.\n");
        builder.append("RULES:\n");
        builder.append("1. Use the Google Maps tool to estimate travel time and distance between stops.\n");
        builder.append("2. Do not exceed ").append(maxStops).append(" stops in any route.\n");
        builder.append("3. If only the new label is within the radius, set createNewRoute=true and ");
        builder.append("orderedUuids=[newLabelUuid].\n");
        builder.append("4. Add the new label to the route that best fits its deadline and location.\n");
        builder.append("5. Order the chosen route's stops by address/location and deadlines.\n");
        builder.append("6. orderedUuids must include every stop for the chosen route plus the new label.\n");
        builder.append("OUTPUT JSON only with routeId, createNewRoute, orderedUuids, notes.\n\n");

        builder.append("NEW LABEL:\n");
        builder.append("- uuid: ").append(newLabel.getUuid()).append('\n');
        builder.append("  name: ").append(safe(newLabel.getRecipientName())).append('\n');
        builder.append("  address: ").append(safe(newLabel.getAddress())).append('\n');
        builder.append("  city: ").append(safe(newLabel.getCity())).append('\n');
        builder.append("  state: ").append(safe(newLabel.getState())).append('\n');
        builder.append("  zip: ").append(safe(newLabel.getZipCode())).append('\n');
        builder.append("  deadline: ").append(formatInstant(newLabel.getDeliverBy())).append('\n');
        ...
        for (Map.Entry<String, List<ShippingLabelMetaDataEntity>> entry : routes.entrySet()) {
            ...
            builder.append("- routeId: ").append(entry.getKey()).append('\n');
            List<ShippingLabelMetaDataEntity> stops = entry.getValue();
            ...
            for (ShippingLabelMetaDataEntity stop : stops) {
                ...
                builder.append("  stop:\n");
                builder.append("    uuid: ").append(safe(stop.getUuid())).append('\n');
                builder.append("    address: ").append(safe(stop.getAddress())).append('\n');
                builder.append("    city: ").append(safe(stop.getCity())).append('\n');
                builder.append("    state: ").append(safe(stop.getState())).append('\n');
                builder.append("    zip: ").append(safe(stop.getZipCode())).append('\n');
                builder.append("    deadline: ").append(formatInstant(stop.getDeliverBy())).append('\n');
            }
        }

        builder.append("\nOUTPUT FORMAT:\n");
        builder.append("{\"routeId\":\"RTE-123\",\"createNewRoute\":false,\"orderedUuids\":[\"uuid\"],\"notes\":\"string\"}\n");
        return builder.toString();
    }
    ...
    private Schema buildIntakeAssignmentSchema() {
        Map<String, Schema> properties = new HashMap<>();
        properties.put("routeId", Schema.builder()
                .type(Type.Known.STRING)
                .nullable(true)
                .build());
        properties.put("createNewRoute", Schema.builder()
                .type(Type.Known.BOOLEAN)
                .build());
        ...
        return Schema.builder()
                ...
                .required(List.of("createNewRoute", "orderedUuids"))
                .build();
    }
