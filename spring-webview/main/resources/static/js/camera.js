(async function() {
    ...
    try {
        ...
        API_INTAKE_STATUS = config.endpoints.intakeStatus;
    ...
    const intakeDetailsModal = document.querySelector('[data-intake-details-modal]');
    const intakeDetailsBackdrop = document.getElementById('intakeDetailsBackdrop');
    const intakeDetailsStatus = document.querySelector('[data-intake-details-status]');
    const intakeDetailsClose = document.querySelector('[data-intake-details-close]');
    const intakeDetailsConfirm = document.querySelector('[data-intake-details-confirm]');
    ...
    function pollIntakeStatus() {
        if (!API_INTAKE_STATUS || !scanSessionId) {
            return Promise.resolve(null);
        }
        const now = Date.now();
        if (now - lastIntakePollAt < MIN_POLL_GAP_MS) {
            return Promise.resolve(null);
        }
        lastIntakePollAt = now;
        const url = new URL(API_INTAKE_STATUS, window.location.origin);
        url.searchParams.set("scanSessionId", scanSessionId);
    ...
    function updateStateBadge(payload) {
        ...
        if (payload && (payload.intakeStatus === "already_scanned"
            || payload.intakeStatus === "processing"
            || payload.intakeStatus === "address_verifying"
            || payload.intakeStatus === "verifying-address")) {
            return;
        }
        if (processingPaused) {
            return;
    ...
    function updateProcessingPause(payload) {
        if (!payload || cameraType !== "INTAKE") {
            if (processingPaused) {
                processingPaused = false;
            }
            return;
        }
        const shouldPause = payload.intakeStatus === "processing"
            || payload.intakeStatus === "address_verifying"
            || payload.intakeStatus === "verifying-address";
    ...
    function handleIntakeDetails(payload) {
        if (!intakeDetailsModal || !payload || cameraType !== "INTAKE") {
            return;
        }
        ...
        const isProcessing = payload.intakeStatus === "processing"
            || payload.geminiResponseState === "RESPONDING";
        const isVerifying = payload.intakeStatus === "address_verifying"
            || payload.intakeStatus === "verifying-address";
        ...
        if (!intakeDetailsModal.hasAttribute("hidden")) {
            return;
        }
        pendingIntakePayload = payload;
        const addressParts = [payload.city, payload.state, payload.zipCode]
            .filter(Boolean)
            .map(part => String(part).trim())
            .filter(part => part.length);
        const cityStateZip = addressParts.length ? addressParts.join(", ") : "";
        ...
        intakeDetailsModal.querySelectorAll("[data-intake-detail]").forEach((node) => {
            const key = node.getAttribute("data-intake-detail");
            if (!key) return;
            const value = details[key];
            node.textContent = value && String(value).trim().length ? value : "N/A";
        });
        if (intakeDetailsStatus) {
            const statusText = payload.existingLabel
                ? "Label already exists in the system."
                : (payload.notes || "");
            intakeDetailsStatus.textContent = statusText;
            intakeDetailsStatus.hidden = !statusText;
        }
        if (intakeDetailsConfirm) {
            intakeDetailsConfirm.hidden = !!payload.existingLabel;
        }

        intakeDetailsModal.removeAttribute("hidden");
        document.body.classList.add("scan-modal-open");
        loopPaused = true;
        requestAnimationFrame(() => intakeDetailsModal.classList.add("open"));
    }

    function closeIntakeDetails() {
        if (!intakeDetailsModal) return;
        const shouldClosePopup = pendingIntakePayload && pendingIntakePayload.existingLabel;
        intakeDetailsModal.classList.remove("open");
        document.body.classList.remove("scan-modal-open");
        loopPaused = false;
        setTimeout(() => {
            intakeDetailsModal.setAttribute("hidden", "hidden");
        }, 220);
    ...
    if (intakeDetailsClose) {
        intakeDetailsClose.addEventListener("click", closeIntakeDetails);
    }

    if (intakeDetailsBackdrop) {
        intakeDetailsBackdrop.addEventListener("click", closeIntakeDetails);
    }

    if (intakeDetailsConfirm) {
        intakeDetailsConfirm.addEventListener("click", () => {
            ...
            if (intakeDetailsStatus) {
                intakeDetailsStatus.textContent = "Processing...";
                intakeDetailsStatus.hidden = false;
            }
            ...
            submitIntake(payload)
                ...
                .catch(() => {
                    if (intakeDetailsStatus) {
                        intakeDetailsStatus.textContent = "Unable to confirm intake.";
                        intakeDetailsStatus.hidden = false;
                    }
                })
                .finally(closeIntakeDetails);
        });
    }
